...
LUA_API int lua_checkstack(lua_State *L, int n)
{
  ...
  { /* no; need to grow stack */
    ...
    res = (inuse > LUAI_MAXSTACK - n) ? 0 : luaD_growstack(L, n, 0); /* can grow without overflow? */ /* try to grow stack */
  }
...
LUA_API int lua_dump(lua_State *L, lua_Writer writer, void *data, int strip)
{
  ...
  api_checknelems(L, 1);
  o = s2v(L->top - 1);
  status = isLfunction(o) ? luaU_dump(L, getproto(o), writer, data, strip) : 1;
  lua_unlock(L);
  return status;
}
...
LUA_API void lua_toclose(lua_State *L, int idx)
{
  ...
  if (!hastocloseCfunc(nresults))
    L->ci->nresults = codeNresults(nresults); /* function not marked yet? mark it */
